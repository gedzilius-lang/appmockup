...
# Overwrite OS homepage with guest checkin box + events list
cat > /opt/pwl-os/os/app/page.js <<'PAGE'
...
# Add /guest panel (client page)
mkdir -p /opt/pwl-os/os/app/guest
cat > /opt/pwl-os/os/app/guest/page.js <<'PAGE'
"use client";
...
# Ensure client can see API base url
...
echo "✅ OS Phase 1.2: /guest panel + guest check-in + me status + tracking"
