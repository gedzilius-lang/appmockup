...
# 2) Patch server.js import/register
# (overwrite the top part safely by rewriting the full file if you want minimal risk)
# We'll do a simple in-place transformation: remove fastify-cors usage and use @fastify/cors.

# If server.js is small enough, overwrite it with a corrected version (recommended).
cat > server.js <<'JS'
...
echo "✅ API patched: using @fastify/cors (Fastify v4 compatible)."
